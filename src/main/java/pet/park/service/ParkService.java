...
public class ParkService {
	...
	private void setFieldsContributor(Contributor contributor, ContributorData contributorData) {
		...
		if (Objects.isNull(contributorId)) {
			...
			if(opContrib.isPresent()) {
				throw new DuplicateKeyException(
						"Contribtor with email " + contributorEmail + " already exists");
			}
	...
	private PetPark findPetParkById(Long petParkId) {
		return petParkDao.findById(petParkId).orElseThrow(() -> new NoSuchElementException("Pet Park with ID=" + petParkId + " does not exsist."));
	}
